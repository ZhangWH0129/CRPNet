...
import numpy as np
import ZReg.CrossAtt as CrossAtt
import ZReg.configs_TransMatch as configs
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, y, mask=None):
        ...
        # q2 = qkv[0]
        # k2, v2 = qkv2[1], qkv2[2]  # make torchscript happy (cannot use tensor as tuple)
        #
        # q2 = q2 * self.scale
...
class CrossSwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, y, mask_matrix):
        ...
        y_windows = y_windows.view(-1, self.window_size[0] * self.window_size[1] * self.window_size[2], C)  # nW*B, window_size*window_size*window_size, C
        # W-MSA/SW-MSA
        attn_windows1, attn_windows2 = self.attn(x_windows, y_windows, mask=attn_mask)  # nW*B, window_size*window_size*window_size, C

        # merge windows
        attn_windows1 = attn_windows1.view(-1, self.window_size[0], self.window_size[1], self.window_size[2], C)
        shifted_x = window_reverse(attn_windows1, self.window_size, Hp, Wp, Tp)  # B H' W' L' C
        # shifted_y = window_reverse(attn_windows, self.window_size, Hp, Wp, Tp)  # B H' W' L' C
        attn_windows2 = attn_windows2.view(-1, self.window_size[0], self.window_size[1], self.window_size[2], C)
        shifted_y = window_reverse(attn_windows2, self.window_size, Hp, Wp, Tp)
...
class CrossBasicLayer(nn.Module):
    """ A basic Swin Transformer layer for one stage.
    Args:
        dim (int): Number of feature channels
        depth (int): Depths of this stage.
    ...
    def forward(self, x, y, H, W, T):
        ...
        for blk in self.blocks:
            ...
            if self.use_checkpoint:
                ...
                x, y = blk(x, y, attn_mask)
        return x, y, H, W, T, x, H, W, T
...
class CrossSwinTransformer(nn.Module):
    ...
    def __init__(self, pretrain_img_size=224,
        ...
        self.layers = nn.ModuleList()
        for i_layer in range(1):
            layer = CrossBasicLayer(dim=int(embed_dim * 2 ** i_layer),
                                depth=depths[i_layer],
                                num_heads=num_heads[i_layer],
                                window_size=window_size,
                                mlp_ratio=mlp_ratio,
    ...
    def forward(self, x, y):
        ...
        for i in range(1):  # num_layers  = len(depths)  = 4
            layer = self.layers[i]
            x_out, y_out, H, W, T, x, Wh, Ww, Wt = layer(x, y, Wh, Ww, Wt)
            # print('ok')
            if i in self.out_indices:
                norm_layer = getattr(self, f'norm{0}')
                # print(norm_layer)
                # norm_layer = nn.LayerNorm(self.dim_diy, eps=1e-05, elementwise_affine=True)
                x_out = norm_layer(x_out)
                y_out = norm_layer(y_out)
                # print('***', x_out.shape)
                # print('The num_features:', self.num_features)

                out_x = x_out.view(-1, H, W, T, self.num_features).permute(0, 4, 1, 2, 3).contiguous()
                out_y = y_out.view(-1, H, W, T, self.num_features).permute(0, 4, 1, 2, 3).contiguous()
        return out_x, out_y
